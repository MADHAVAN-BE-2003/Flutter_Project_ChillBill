...
import 'package:flutter_application_1/createpdf/api/pdf_invoice_api.dart';
import 'package:flutter_application_1/createpdf/api/pdf_invoice_print.dart';
...
import 'package:flutter_application_1/pages/sales/productgrid.dart';
import 'package:intl/intl.dart';
import 'package:flutter_application_1/common/apicall.dart';
// import 'package:flutter_application_1/pages/sales/Productgridk.dart';
// import 'package:flutter_application_1/pages/sales/Productgridkk.dart';
import 'package:localstorage/localstorage.dart';

// final storage = new LocalStorage('store_product_invoice');
...
class Salesinvoice extends StatefulWidget {
  ...
  final dynamic itemslistarray1;
  final dynamic client;
  final dynamic client_name;
  final dynamic company_contact;
  final dynamic company_name;
  final dynamic company_address;
  ...
  final dynamic inv_tax_status;
  final dynamic inv_dis_status;
  final dynamic grandTotal;
  final dynamic payment_received;
  final dynamic payment_balance;
  final dynamic payment_words;
...
late String overalltotal = _total.toString();
List<Item> generateItems(int numberOfItems) {
  ...
  return List<Item>.generate(itemslistarray.length, (int index) {
    return Item(
      ...
      headerdis: fruitMap[index]['headerdis'].toString(),
...
class _SalesinvoiceState extends State<Salesinvoice> {
  ...
  createprint() async {
    ...
    if (itemslistarray.length != 0) {
      ...
      print(widget);
      var company_name = widget.company_name;
      var company_address = widget.company_address;
      var payment_method = widget.payment_method;
      var customername = widget.customername;
      var customeraddress = widget.customeraddress;
      var customerphone = widget.customerphone;

      var taxRate = widget.taxRate;
      var discountRate = widget.discountRate;
      var inv_tax_status = widget.inv_tax_status;
      var inv_dis_status = widget.inv_dis_status;
      var grandTotal = widget.grandTotal;
      var payment_received = widget.payment_received;
      var payment_balance = widget.payment_balance;
      var payment_words = widget.payment_words;
      var invoicenumber = widget.invoicenumber;
      var invoiceDate = widget.invoiceDate;

      var invoice_items = itemslistarray.map((item) {
        print(item['headerdis']);
        var totalScores;
        if (item['headerdis'] == 0) {
          totalScores = '0.0';
        } else {
          totalScores = item['headerdis'];
        }
        ...
        return InvoiceItem(
          ...
          date: DateTime.now(),
          quantity: int.parse(item['headerquentity']),
          vat: double.parse(item['headertax']),
          unitPrice: lat2,
        );
      ...
      if (thermalprint == true) {
        thermalPrint?.getprint(itemslistarray, pathImage!, billing_name,
            widget.company_contact, widget.client_name, widget.client);
      }
      //  pathImage = '$dir/$filename';
...
// await file.writeAsBytes(await doc.save());
  ...
  Widget build(BuildContext context) {
    // itemslistarray = widget.itemslistarray1;
    itemslistarray = widget.itemslistarray1;

    // var total = itemslistarray.length != 0
    //     ? itemslistarray
    //         .map<double>((p) => p.headertotal)
    //         .reduce((a, b) => a + b)
    //     : '0';
    // // print(widget.itemslistarray1);
    // setState(() {
    //   // overalltotal = _total.toString();
    //   overalltotal = total.toString();
    // });
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () async {
          ...
          if (itemslistarray.length > 0) {
            ...
            var finalvalue = jsonEncode(formvalues);
            // await saleslogin(finalvalue).then((val) {
            //   // print(val);
            //   // ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget _buildPanel() {
    ...
    itemslistarray.forEach((item) {
      //                             var sales_price =item["headerprice"];
      //                             var sales_tax = item["headertax"];
      //                             var sales_dis = item["headerdis"];

      //                             var dis_price = int.parse(sales_price) *
      //                                 (int.parse(sales_dis) / 100);
      //                             var tax_price =
      //                                 (int.parse(sales_price) - dis_price) *
      //                                     (int.parse(sales_tax) / 100);
      //                             var autal_price =
      //                                 (int.parse(sales_price) - dis_price) +
      //                                     tax_price.round();

      // var total =
      //                                 autal_price * int.parse(item["headerquentity"]);
      // print(item["headertotal"]);
      //getting the key direectly from the name of the key
      var headertotal = item["headertotal"].toString();
      // headertotal = headertotal.toStringAsFixed(2);
      // print(headertotal);
      totalScores += double.parse(headertotal);
    });
    ...
    setState(() {
      overalltotal = totalScores.toStringAsFixed(2);
    });
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        var headerprice = double.parse(itemslistarray[index]['headerprice'])
            .toStringAsFixed(2);
        // print(itemslistarray[index]['headertotal']);
        // var headertotal = itemslistarray[index]['headertotal'];
        var headertotal =
            double.parse(itemslistarray[index]['headertotal'].toString())
                .toStringAsFixed(2);
        // .toStringAsFixed(2);
        // headertotal = headertotal.toStringAsFixed(2);
        // headerprice = headerprice.toStringAsFixed(2);
...
class BackendService {
  static Future<List<Map<String, String>>> getSuggestions(String query) async {
    ...
    final response = await http.post(
      ...
      // body: {
      //   'name': query,
      //   'report_action': 'customernamelistapp',
      //   'master': loginuserid,
      //   'role': loginrole,
      //   'isappp': "okay",
      // },
                          ...
                          style: ButtonStyle(
                              ...
                              shape: MaterialStateProperty.all<
                                  ...
                                  RoundedRectangleBorder(
                                      ...
                                      side: BorderSide(color: Colors.red)))),
                          onPressed: () async {
                            final _SalesinvoiceState ani =
                                new _SalesinvoiceState();
                            // @override
                            // Widget build(BuildContext context) {
